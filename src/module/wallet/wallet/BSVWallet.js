...
export default class BSVWallet extends Wallet {
  BSV = new BSV();
  ...
  static fromBTCWallet = async (BTCWallet, pwd) => {
    const wallet = new BSVWallet(BTCWallet);
  ...
  calculateMaximumAmount = async feePerByte => {
    ...
    const maximum = new BigNumber(this.BSV.balance + "").minus(fee);
